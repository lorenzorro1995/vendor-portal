<x-app-layout>
    <h1 class="text-2xl font-semibold text-gray-800 dark:text-white mb-6">Edit Vendor Profile: {{ $user->name }}</h1>

    <div class="p-4 sm:p-8 bg-white dark:bg-gray-900 shadow sm:rounded-lg">
        <form method="POST" action="{{ route('admin.vendors.update', $user) }}">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <x-text-input id="company_name" class="block mt-1 w-full" type="text" name="company_name" :value="old('company_name', $user->vendorProfile->company_name)" required />
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    <x-input-label for="company_phone" :value="__('Company Phone Number')" />
                    <x-text-input id="company_phone" class="block mt-1 w-full" type="tel" name="company_phone" :value="old('company_phone', $user->vendorProfile->company_phone)" />
                </div>
                <div>
                    <x-input-label for="company_email" :value="__('Company Email Address')" />
                    <x-text-input id="company_email" class="block mt-1 w-full" type="email" name="company_email" :value="old('company_email', $user->vendorProfile->company_email)" />
                </div>
                <div>
                    <x-input-label for="company_fax" :value="__('Company Fax Number')" />
                    <x-text-input id="company_fax" class="block mt-1 w-full" type="text" name="company_fax" :value="old('company_fax', $user->vendorProfile->company_fax)" />
                </div>
                <div>
                    <x-input-label for="company_registration_number" :value="__('Company Registration Number')" />
                    <x-text-input id="company_registration_number" class="block mt-1 w-full" type="text" name="company_registration_number" :value="old('company_registration_number', $user->vendorProfile->company_registration_number)" />
                </div>
                <div>
                    ...
                    <select id="company_type" name="company_type" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 focus:border-indigo-500 rounded-md shadow-sm">
                        @php
                            $companyTypes = ['Private Limited', 'Corporation', 'Company Limited', 'Enterprise', 'Sole Proprietorship'];
                            $selectedType = old('company_type', $user->vendorProfile->company_type);
                        @endphp
            ...
            <div class="mb-6">
                ...
                <textarea id="company_profile" name="company_profile" rows="4" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 focus:border-indigo-500 rounded-md shadow-sm">{{ old('company_profile', $user->vendorProfile->company_profile) }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <x-text-input id="address_1" class="block mt-1 w-full" type="text" name="address_1" :value="old('address_1', $user->vendorProfile->address_1)" required />
                </div>
                <div>
                    <x-input-label for="address_2" :value="__('Address 2 (Optional)')" />
                    <x-text-input id="address_2" class="block mt-1 w-full" type="text" name="address_2" :value="old('address_2', $user->vendorProfile->address_2)" />
                </div>
                <div>
                    <x-input-label for="address_3" :value="__('Address 3 (Optional)')" />
                    <x-text-input id="address_3" class="block mt-1 w-full" type="text" name="address_3" :value="old('address_3', $user->vendorProfile->address_3)" />
                </div>
                <div>
                    <x-input-label for="city" :value="__('City')" />
                    <x-text-input id="city" class="block mt-1 w-full" type="text" name="city" :value="old('city', $user->vendorProfile->city)" required />
                </div>
                <div>
                    <x-input-label for="state" :value="__('State / Province')" />
                    <x-text-input id="state" class="block mt-1 w-full" type="text" name="state" :value="old('state', $user->vendorProfile->state)" required />
                </div>
                <div>
                    <x-input-label for="zip_code" :value="__('Zip / Postal Code')" />
                    <x-text-input id="zip_code" class="block mt-1 w-full" type="text" name="zip_code" :value="old('zip_code', $user->vendorProfile->zip_code)" required />
                </div>
                <div>
                    ...
                    <select id="country" name="country" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 focus:border-indigo-500 rounded-md shadow-sm" required>
                        @foreach ($countries as $country)
                            <option value="{{ $country }}" @selected(old('country', $user->vendorProfile->country) == $country)>{{ $country }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div x-data='directorManager(@json(old("directors", $user->vendorProfile->directorShareholders)))' class="mt-6 pt-6 border-t dark:border-gray-700">
                <h3 class="font-semibold text-lg text-gray-800 dark:text-white mb-2">Director and Shareholder Information</h3>
                <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">Provide complete and accurate details.</p>
            ...
            <div class="flex items-center justify-end mt-6">
                <a href="{{ route('admin.vendors.show', $user) }}" class="text-sm text-gray-600 dark:text-gray-400 hover:underline mr-4">Cancel</a>
